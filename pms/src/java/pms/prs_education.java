...
public String create_prs_education()
 {
    String query="INSERT INTO `prs_education`(`usercode`, `ug`, `pg`, `hsc`,`sslc`) VALUES ('"+usercode +"','"+ ug +"','"+ pg +"','"+ hsc +"','"+ sslc+"')";
    String message=super.tableinsertupdatedelete(query);
    return message;
 }
 public String update_prs_education()
 {
    String query="UPDATE `prs_education` SET `usercode`='"+ usercode +"',`ug`='"+ ug +"',`pg`='"+ pg +"',`hsc`='"+ hsc +"',`sslc`='"+ sslc +"' WHERE id='"+ id+"'";
    String message=super.tableinsertupdatedelete(query);
    return message;
 }
